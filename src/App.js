...
function App() {
  ...
  const TextInput = () => {
    ...
    return (
      ...
      >
        {loading ? <CircularProgress color="inherit" /> : <>Ask</>}
      </Button>
        {/* <p>prompt: {prompt}</p> */}
      </div>
       );
  ...
  const fetchresponse = async () => {
    ...
    if (tabInnerHtmlText === null) {
      setText('No web content found')
      setLoading(false);
      return
    } 
    const context = getValidLengthText(tabInnerHtmlText[0])
    const response = await openai.ChatCompletion.create({
      model:"gpt-3.5-turbo",
      prompt: prompt + " and use this as context: " + context, 
      temperature: 0.7,
      top_p: 1.0,
  ...
  const fetchSummary = async () => {
    ...
    const tabInnerHtmlText = await getCurrentTabHtml();
    const validPrompt = getValidLengthText(tabInnerHtmlText[0])
